...
void main(){
	FILE *sav, *in; 
	FILE *game; // creando variable para crear archivo juego.txt
	...
	do{
		...
		if(option==3&&p1==2){
			game=fopen("juego.txt","w");// abriendo el archivo juego.txt en modo escritura
			do{
				system("cls");
				...
				switch (tecla){
					case '+':
						...
						getch();
//----------------------------- Guardando posicion por cada jugador en juego.txt --------------------------------------------
						fprintf(game,"%s:casilla %d\n", gamer[r].nombre, gamer[r].avance);
//---------------------------------------------------------------------------------------------------------------------------
						if(n!=1){
							temp=r;
							r=(r+1)%2;
			...
			fclose(game);// cerrando el archivo juego.txt
			option=6;
		}
...
int registrarGanador(JugadorT gam[], int turno){
	JugadorT temp[200];
	int k=0, i, flag=0;
	int suma=0; //declarando la variable suma para que lamcene el numero de veces al momento de imprimir el top
	char cadena[30];
	FILE *top, *temptop;
	...
	if(temptop==NULL){
		...
		if(k>9)
			k=9;
		for(i=k;i>=0;i--){
			suma+=temp[i].veces;        //acumulando el numero de veces ganado de cada jugador para mostrar el total al final
			fprintf(top,"\n%s %d", temp[i].nombre, temp[i].veces);
		}
		fprintf(top,"\nTotal: %d", suma);
	}
